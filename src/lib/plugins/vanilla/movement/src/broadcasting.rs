...
pub fn broadcast_position_updates(
    ...
    for event in events.read() {
        ...
        // Vanilla rule: Broadcast to ALL players (broadcaster filters out the moving player)
        if is_teleport {
...
pub fn broadcast_rotation_updates(
    ...
    for event in events.read() {
        // Vanilla rule: Broadcast to ALL players (broadcaster filters out the rotating player)
        api.broadcast_movement_all(
            event.player,
...
pub fn broadcast_head_rotation(
    ...
    for event in events.read() {
        // Vanilla rule: Broadcast to ALL players (broadcaster filters out the rotating player)
        api.broadcast_head_rotation_all(event.player, event.rotation.yaw);
